...
public class DialogNovaVenda extends JDialog {
    ...
    public DialogNovaVenda(Frame parent, VendaController vc, Usuario user) {
        ...
        JButton btnFinalizar = new JButton("CONCLUIR VENDA");
        ...
        panelRight.add(new JScrollPane(tabelaItens), BorderLayout.CENTER);
    ...
    private void carregarClientes() {
        ...
        List<Cliente> lista = vendaController.buscarClientesParaVenda();
        for(Cliente c : lista) cbClientes.addItem(c);
    }

    private void carregarProdutos() {
        cbProdutos.removeAllItems();
        List<Produto> lista = vendaController.buscarProdutosAtivos();
    ...
    private void cadastrarClienteRapido() {
        ...
        if(nome != null && !nome.isEmpty()) {
            vendaController.cadastrarClienteRapido(nome);
            carregarClientes();
    ...
    private void adicionarItem() {
        try {
            ...
            if (p == null) return;
            if (p.getQuantidadeEstoque() <= 0 || qtd > p.getQuantidadeEstoque()) {
